...
pipeline {
    ...
    stages {
        ...
        stage('Checkout To Mcroservice Branch'){
            steps{
                git branch: 'app-shipping-service', url: 'https://github.com/EseoheB/realworld-microservice-projects.git'
            }
        }
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t eseoheb/shippingservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker eseoheb/shippingservice:latest || true" 
            }
        }
        ...
        //         script {
        //             withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
        //                 sh "docker push EseoheB/shippingservice:latest "
        //             }
        //         }
        //     }
        // }
    ...
    always {
        ...
        slackSend channel: '#rib--multi-microservices-alert', //update and provide your channel name
